...
install_script_version=1.18.0.post
...
if [ -n "$fips_mode" ]; then
    ...
    if [[ -n "$agent_minor_version_without_patch" ]] && [[ "${agent_minor_version_without_patch}" -lt 41 ]]; then
        ERROR_MESSAGE="FIPS mode is only available since version 1.41.0 and requested minor version is $agent_minor_version"
        ERROR_CODE=$INVALID_PARAMETERS_CODE
        printf "\033[31m$ERROR_MESSAGE\033[0m\n"
        report_telemetry
        exit 1;
...
EOF
  ...
  if [ "$DD_RUNTIME_SECURITY_CONFIG_ENABLED" == "true" ]; then
    ...
    ensure_config_file_exists "$sudo_cmd" "$security_agent_config_file" "root"

    printf "\033[34m\n* Enabling runtime security in configuration\n\033[0m\n"
    $sudo_cmd sh -c "sed -i 's/^#\s*runtime_security_config:/runtime_security_config:/' $security_agent_config_file $system_probe_config_file"
    $sudo_cmd sh -c "sed -i '/^runtime_security_config:/,// s/\(\s\+\)#\s*enabled:\s*false/\1enabled: true/' $security_agent_config_file $system_probe_config_file"
  fi
  ...
  if [ "$DD_COMPLIANCE_CONFIG_ENABLED" == "true" ]; then
    ...
    $sudo_cmd sh -c "sed -i 's/^#\s*compliance_config:/compliance_config:/' $security_agent_config_file"
    $sudo_cmd sh -c "sed -i '/^compliance_config:/,// s/\(\s\+\)#\s*enabled:\s*false/\1enabled: true/' $security_agent_config_file"
  fi
fi
