...
while getopts "f:m:a:s:n" opt; do
  case $opt in
    f) # datadog-agent, datadog-iot-agent, datadog-dogstatsd
      export FLAVOR="$OPTARG"
      ;;
    m) # install, upgrade5, upgrade6, upgrade7
      export MODE="$OPTARG"
      ;;
    a) # Needs to be valid, at least for the upgrade5 scenario
      export API_KEY="$OPTARG"
      ;;
    s) # To test a non-published script
      SCRIPT_URL="$OPTARG"
      ;;
    n) # To prevent eventual cleanup, to test install_script won't override existing configuration
      NO_FLUSH="true"
      break # to make the option work without argument, must be the last one
      ;;
    \?)
      echo "Unknown option $OPTARG"
      echo "Usage $0 -f FLAVOUR -m MODE -a API_KEY [-s SCRIPT_URL] [-n (NO_FLUSH)]"
      exit 1
      ;;
  esac
done
