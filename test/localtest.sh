...
echo "testing apm library installation"
if [ -n "$DD_APM_INSTRUMENTATION_ENABLED" ] || [ "${SCRIPT_FLAVOR}" == "docker_injection" ]; then
  echo "aha"
  test -L /opt/datadog-packages/datadog-apm-inject/stable
  echo "ys1"
  test -L /opt/datadog-packages/datadog-apm-library-dotnet/stable
  test -L /opt/datadog-packages/datadog-apm-library-java/stable
  test -L /opt/datadog-packages/datadog-apm-library-js/stable
  test -L /opt/datadog-packages/datadog-apm-library-python/stable
  test -L /opt/datadog-packages/datadog-apm-library-ruby/stable
